    ...
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-col gap-4 my-8">
            ...
            <div className="flex gap-2">
              {productData.sizes.map((item, inx) => (
                <button onClick={() => setSize(item)} key={inx} className={`border border-gray-300 py-2 px-4 bg-gray-100 ${item === size ? 'border-orange-300' : ''}`}>{item}</button>
              ))}
            </div>
          </div>
      ...
      <div className="mt-20">
        <div className="flex gap-1">
          ...
          <p className="border px-5 py-3 text-sm cursor-pointer">Reviews (118)</p>
        </div>
